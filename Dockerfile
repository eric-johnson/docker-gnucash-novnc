...
RUN apt-get update -y && \
	...
	apt-get install -y \
		...
		aqbanking-tools && \
	wget https://github.com/Gnucash/gnucash/releases/download/2.6.19/gnucash-2.6.19.tar.bz2 && \
	tar xvjf gnucash-2.6.19.tar.bz2 && rm gnucash-2.6.19.tar.bz2 && mv gnucash-2.6.19/* . && rmdir gnucash-2.6.19 && \
